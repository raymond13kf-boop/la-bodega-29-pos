...
export function Ventas() {
  ...
  const handleProcessPayment = async () => {
    ...
    try {
      ...
      // 1.5. Pre-validate stock in real-time in DB to prevent negative stock (Rule 1, 3, 4)
      ...
      if (fetchError || !dbProducts) {
        alert('Error al verificar el stock de los productos. Inténtelo de nuevo.');
        setIsProcessing(false);
        return;
      }
      ...
      for (const cartItem of cart) {
        ...
        const currentDbStock = dbProduct ? dbProduct.stock : 0;
        if (currentDbStock - cartItem.quantity < 0) {
          insufficientStockItems.push(`${cartItem.name} (Disponible: ${currentDbStock}, Solicitado: ${cartItem.quantity})`);
        }
      }

      if (insufficientStockItems.length > 0) {
        alert(`No se puede procesar la venta. Los siguientes productos no tienen suficiente stock en la base de datos:\n\n${insufficientStockItems.join('\n')}`);
        console.warn('Intento de venta con stock insuficiente/negativo abortado:', insufficientStockItems);
        setIsProcessing(false);
        // Refresh products catalog to show the actual stock levels in UI
        await fetchProducts();
        return;
      }
      ...
      // 5. Update stock safely in DB (Rule 1, 3)
      for (const item of cart) {
         const dbProduct = dbProducts.find(p => p.id === item.id);
         const currentDbStock = dbProduct ? dbProduct.stock : 0;
         const newStock = Math.max(0, currentDbStock - item.quantity); // Guarantee minimum 0
      ...
      setAmountPaid('');
      setEntryDate('');
      // Sync local catalog with actual DB values
      await fetchProducts();
    } catch (error) {
      console.error(error);
